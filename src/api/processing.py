...
def require_admin_permission(user: dict = Depends(get_current_user)) -> dict:
    ...
    if 'Admin' not in user.get('roles', []):
        raise HTTPException(
            ...
            detail="Admin role required to view processing status"
        )

    if 'canManagePipeline' not in user.get('permissions', []):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="canManagePipeline permission required to view processing status"
        )
...
async def get_processing_status(
    ...
    if ingestion_job.user_id != user['user_id'] and 'Admin' not in user.get('roles', []):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
